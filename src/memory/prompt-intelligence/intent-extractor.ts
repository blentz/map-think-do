...
export class IntentExtractor {
  ...
  private assessPromptComplexity(prompt: string): number {
    ...
    const techCount = technicalTerms.filter(term => new RegExp(term, 'i').test(prompt)).length;
  ...
  async getExtractionStats(): Promise<{
    ...
    return {
      totalPatterns:
        this.objectivePatterns.length +
        this.constraintPatterns.length +
        this.requirementPatterns.length +
        this.outputTypePatterns.length,
