...
export class SimilarityDetector {
  ...
  private readonly technicalTerms = new Set([
    // Core tech terms
    'api', 'database', 'algorithm', 'architecture', 'framework', 'performance',
    'security', 'scalability', 'integration', 'deployment', 'configuration',
    'authentication', 'authorization', 'middleware', 'microservices', 'docker',
    'kubernetes', 'aws', 'azure', 'gcp', 'cloud', 'server', 'client',
    // Programming languages
    'typescript', 'javascript', 'python', 'java', 'golang', 'rust', 'php', 'ruby',
    'csharp', 'cpp', 'sql', 'html', 'css', 'json', 'xml', 'yaml',
    // Frameworks & libraries
    'react', 'vue', 'angular', 'node', 'express', 'fastapi', 'django', 'flask',
    'spring', 'laravel', 'rails', 'nextjs', 'nuxt', 'svelte', 'ember',
    // Databases & storage
    'postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch', 'kafka', 'sqlite',
    'cassandra', 'dynamodb', 'firebase', 'prisma', 'orm', 'nosql',
    // Development terms
    'testing', 'debugging', 'refactoring', 'optimization', 'build', 'compile',
    'deploy', 'ci', 'cd', 'devops', 'git', 'github', 'gitlab', 'npm', 'yarn',
    // Concepts
    'async', 'sync', 'promise', 'callback', 'event', 'handler', 'component',
    'module', 'class', 'function', 'method', 'variable', 'constant', 'interface'
  ]);
  ...
  async findSimilarPrompts(
    ...
    for (const candidate of candidatePrompts) {
      ...
      const domainScore = this.calculateDomainSimilarity(targetTechnicalTerms, candidateTechnicalTerms);
  ...
  private calculateStructuralSimilarity(prompt1: string, prompt2: string): number {
    ...
    const sentenceSimilarity = 1 - Math.abs(sentences1 - sentences2) / Math.max(sentences1, sentences2);
  ...
  private extractActionVerbs(text: string): string[] {
    const actionVerbs = [
      'implement', 'create', 'build', 'develop', 'design', 'write', 'make',
      'fix', 'debug', 'solve', 'resolve', 'address', 'repair',
      'optimize', 'improve', 'enhance', 'upgrade', 'refactor',
      'analyze', 'understand', 'explain', 'review', 'examine',
      'test', 'validate', 'verify', 'check', 'ensure',
      'add', 'remove', 'delete', 'modify', 'update', 'change',
   ...
   */
  private generateSimilarityReasoning(scores: {
    semanticScore: number;
    structuralScore: number;
    domainScore: number;
    intentScore: number;
    ...
    if (semanticScore > 0.5) reasons.push(`high semantic similarity (${(semanticScore * 100).toFixed(1)}%)`);
    if (domainScore > 0.5) reasons.push(`shared domain knowledge (${(domainScore * 100).toFixed(1)}%)`);
    if (intentScore > 0.5) reasons.push(`similar intent/actions (${(intentScore * 100).toFixed(1)}%)`);
    if (structuralScore > 0.5) reasons.push(`similar structure (${(structuralScore * 100).toFixed(1)}%)`);
