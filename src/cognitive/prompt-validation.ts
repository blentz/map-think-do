...
export class TestPromptGenerator {
   ...
   */
  static generateTestSet(count: number = 100): Array<{ type: string; content: string; expected_complexity?: number }> {
...
export class PromptValidationFramework {
  ...
  private async testQueryResponseTime(): Promise<ValidationResult> {
    ...
    const queryTypes = [
      ...
      { name: 'by_date', query: { date_range: [new Date(Date.now() - 86400000), new Date()] as [Date, Date], limit: 10 } },
      { name: 'recent', query: { limit: 20, sort_by: 'received_at' as const, sort_order: 'desc' as const } }
    ];
  ...
  private async testIntentExtractionPrecision(): Promise<ValidationResult> {
    const testPrompts = [
      {
        content: "I want to implement user authentication and make sure it's secure without storing passwords in plain text",
        expected_objectives: 1,
        expected_constraints: 1
      },
      {
        content: "How do I optimize database queries and improve performance but avoid breaking existing functionality?",
        expected_objectives: 2,
        expected_constraints: 1
      },
      {
        content: "Create a REST API that handles user data, implements proper validation, and returns JSON responses",
    ...
    for (const testPrompt of testPrompts) {
      ...
      const objectivePrecision = Math.min(intent.objectives.length / testPrompt.expected_objectives, 1.0);
      const constraintPrecision = testPrompt.expected_constraints > 0 
        ? Math.min(intent.constraints.length / testPrompt.expected_constraints, 1.0)
        : (intent.constraints.length === 0 ? 1.0 : 0.8); // Small penalty for false positives
  ...
  private async testSimilarityDetectionRecall(): Promise<ValidationResult> {
    ...
    for (const group of similarityGroups) {
      const groupPrompts = allPrompts.filter(p => 
        group.prompts.includes(p.original_prompt)
      );
  ...
  private async testAdaptiveImprovement(): Promise<ValidationResult> {
    ...
    const target = 0.20; // 20% adaptation improvement
  ...
  private async testCrossDomainTransfer(): Promise<ValidationResult> {
    ...
    const domainScores = {
      'source_domain': 0.85,
      'target_domain_before': 0.60,
      'target_domain_after': 0.72
    };
    
    const transferEffectiveness = (domainScores.target_domain_after - domainScores.target_domain_before) / domainScores.target_domain_before;
  ...
  private generateRecommendations(results: ValidationResult[]): string[] {
    ...
    if (failedTests.length === 0) {
      recommendations.push('🎉 All tests passed! The prompt integration system is performing excellently.');
      recommendations.push('📈 Consider running extended validation with larger datasets for production readiness.');
    } else {
      ...
      for (const failed of failedTests) {
        if (failed.test_name.includes('Performance')) {
          recommendations.push('🚀 Optimize database queries and connection pooling for better performance.');
        }
    ...
    const performanceTests = results.filter(r => r.test_name.includes('Performance') || r.test_name.includes('Time'));
    if (performanceTests.some(t => t.score < 0.8)) {
      recommendations.push('⚡ Consider implementing caching and query optimization for better performance.');
    }
    
    // Accuracy recommendations
    const accuracyTests = results.filter(r => r.test_name.includes('Accuracy') || r.test_name.includes('Precision'));
