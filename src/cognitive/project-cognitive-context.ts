...
export class TechnologyCognitiveStrategyMapper {
  ...
  private static readonly TECHNOLOGY_STRATEGIES: Record<string, TechnologyCognitiveStrategy> = {
    ...
    react: {
      ...
      memoryStrategy: {
        project_scope_preference: 0.8,
        pattern_recognition_weight: 0.7,
    ...
    python: {
      ...
      memoryStrategy: {
        project_scope_preference: 0.6,
        pattern_recognition_weight: 0.9,
  ...
  private static combineStrategies(
    strategies: TechnologyCognitiveStrategy[]
    ...
    const combined: TechnologyCognitiveStrategy = {
      ...
      memoryStrategy: {
        project_scope_preference: 0,
        pattern_recognition_weight: 0,
...
export class ProjectLifecycleDetector {
  ...
  static detectLifecyclePhase(
    ...
    if (recentActivity) {
      ...
      if (
        this.containsKeywords(activityText, [
          'architecture',
          'design',
          'plan',
          'structure',
...
export class ProjectCognitiveContextFactory {
  ...
  static async createContext(
    ...
    if (project) {
      ...
      enhancedContext.technologyStrategy =
        TechnologyCognitiveStrategyMapper.getStrategyForTechnologies(
      ...
      if (memoryStore) {
        ...
        enhancedContext.similarProjects = await this.findSimilarProjects(project, memoryStore);
      }
    }
  ...
  private static async buildProjectMemoryContext(
    ...
    try {
      ...
      const recentThoughts =
        (await memoryStore.queryThoughts?.({
   ...
   */
  private static async findSimilarProjects(project: Project, memoryStore: any): Promise<any[]> {
    try {
