...
export class RealSuccessValidator {
  ...
  async validateWithRealMeasurements(): Promise<RealValidationResult> {
    ...
    const overall_score = tier1_score * 0.35 + tier2_score * 0.4 + tier3_score * 0.25;
    const meets_10_10_criteria = tier1_score >= 0.95 && tier2_score >= 0.85 && tier3_score >= 0.8;
  ...
  private async measureRealReasoningImprovement(testCount: number): Promise<number> {
    ...
    for (const prompt of testPrompts) {
      ...
      enhancedScore +=
        (classification.confidence * intent.extraction_confidence) / Math.max(enhancedTime, 1);
    }
  ...
  private calculateTier2ScoreFromMeasurements(
    ...
    const reasoningScore = Math.min(Math.max(reasoningImpr, 0) / 0.2, 1); // Target: >20%
