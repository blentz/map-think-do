...
export class ABTestFramework {
  ...
  async runABTest(config: ABTestConfig): Promise<ABTestResult> {
    ...
    console.error(
      `   Reasoning Improvement: ${result.reasoning_improvement_percentage.toFixed(1)}%`
    );
    console.error(
      `   Classification Improvement: ${result.classification_accuracy_improvement.toFixed(1)}%`
    );
  ...
  private calculateStatistics(
    ...
    const controlConfidence =
      control.reduce((sum, r) => sum + r.confidence_score, 0) / control.length;
    const treatmentConfidence =
      treatment.reduce((sum, r) => sum + r.confidence_score, 0) / treatment.length;

    const controlAccuracy =
      control.reduce((sum, r) => sum + r.classification_accuracy, 0) / control.length;
    const treatmentAccuracy =
      treatment.reduce((sum, r) => sum + r.classification_accuracy, 0) / treatment.length;

    const controlIntent =
      control.reduce((sum, r) => sum + r.intent_extraction_precision, 0) / control.length;
    const treatmentIntent =
      treatment.reduce((sum, r) => sum + r.intent_extraction_precision, 0) / treatment.length;
    ...
    const reasoningImprovement =
      ((treatmentConfidence - controlConfidence) / controlConfidence) * 100;
    const classificationImprovement =
