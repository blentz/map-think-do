...
async function runTests() {
  ...
  try {
    ...
    const testPrompt = "I need to debug a memory leak in my Node.js application that's causing performance issues in production. The heap usage keeps growing and eventually crashes the server.";
    ...
    const performanceTestPrompts = Array.from({ length: 10 }, (_, i) => 
      `Performance test prompt ${i + 1} for measuring storage speed`
    );
    ...
    console.log(`🎯 Target: <50ms per prompt (${avgTime < 50 ? 'PASSED' : 'NEEDS OPTIMIZATION'})\n`);
