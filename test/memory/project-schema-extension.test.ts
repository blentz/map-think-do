  ...
  try {
    ...
    if (testPaths) {
      try {
        ...
        try {
          await client.query('DELETE FROM stored_thoughts WHERE project_id IN (SELECT id FROM projects WHERE directory_path LIKE $1 OR directory_path LIKE $2)', [`/home/user/test-project%`, `/home/user/project-%`]);
          await client.query('DELETE FROM stored_prompts WHERE project_id IN (SELECT id FROM projects WHERE directory_path LIKE $1 OR directory_path LIKE $2)', [`/home/user/test-project%`, `/home/user/project-%`]);
          await client.query('DELETE FROM reasoning_sessions WHERE project_id IN (SELECT id FROM projects WHERE directory_path LIKE $1 OR directory_path LIKE $2)', [`/home/user/test-project%`, `/home/user/project-%`]);
          await client.query('DELETE FROM projects WHERE directory_path LIKE $1 OR directory_path LIKE $2', [`/home/user/test-project%`, `/home/user/project-%`]);
        } finally {
          client.release();
        }
...
async function testProjectCRUD(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  try {
    ...
    try {
      await client.query('DELETE FROM projects WHERE directory_path LIKE $1', [`/home/user/test-project%`]);
    } finally {
      client.release();
    }
  } catch (error) {
...
async function testProjectMetadataExtraction(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  try {
    ...
    assert(extractedMetadata.name.startsWith('test-project-metadata'), 'Project name should start with test-project-metadata');
...
async function testProjectScopedDataStorage(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  assert(updatedProject!.last_activity_at >= project1.last_activity_at, 'Project activity should be updated or equal');
...
async function testErrorHandling(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  try {
    ...
    assert((error as Error).message.includes('creation failed'), 'Should indicate creation failure');
  }
