...
async function testPatternEmbeddingGeneration(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  console.error(`Found ${patternsWithEmbeddings.length} patterns with embeddings out of ${patterns.length} total`);
  
  // Find our test patterns
  const testPatterns = patterns.filter(p => 
    ['debugging-technique', 'problem-solving-pattern', 'systematic-approach'].includes(p.pattern_name)
  );
...
async function testPatternSimilaritySearch(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  try {
    ...
    if (similarPatterns.length > 0) {
      ...
      assert(typeof firstPattern.similarity_score === 'number', 'Similarity score should be number');
      assert(typeof firstPattern.pattern_frequency === 'number', 'Pattern frequency should be number');
      assert(firstPattern.created_at instanceof Date, 'Created at should be Date');
      
      // Verify similarity score is within expected range
      assert(firstPattern.similarity_score >= 0 && firstPattern.similarity_score <= 1, 
        'Similarity score should be between 0 and 1');
      
      console.error(`Top similar pattern: "${firstPattern.pattern_name}" (score: ${firstPattern.similarity_score})`);
    }
    
    console.error('✅ Pattern similarity search validated');
    
  } catch (error) {
    console.error('⚠️ Pattern similarity search not available (likely no embeddings generated yet)');
...
async function testPatternRetrieval(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  assert(frequentPatterns.length <= allPatterns.length, 
    'Frequent patterns should be subset of all patterns');
  
  if (frequentPatterns.length > 0) {
    assert(frequentPatterns.every(p => p.pattern_frequency >= 2), 
  ...
  if (allPatterns.length > 1) {
    for (let i = 1; i < allPatterns.length; i++) {
      assert(allPatterns[i-1].pattern_frequency >= allPatterns[i].pattern_frequency,
...
async function testPatternFrequencyUpdates(store: PostgreSQLMemoryStore): Promise<void> {
  ...
  const updatedDebuggingPattern = updatedPatterns.find(p => p.pattern_name === 'debugging-technique');
  
  if (updatedDebuggingPattern) {
    assert(updatedDebuggingPattern.pattern_frequency > initialFrequency,
      `Pattern frequency should increase from ${initialFrequency} to ${updatedDebuggingPattern.pattern_frequency}`);
    console.error(`✅ Pattern frequency updated: ${initialFrequency} → ${updatedDebuggingPattern.pattern_frequency}`);
  }
