...
import { PersonaConfigAPI } from '../dist/src/cognitive/persona-config-api.js';
import { personaMetrics } from '../dist/src/cognitive/persona-metrics.js';
...
async function testPersonaMetrics() {
  ...
  try {
    ...
    if (updateResult.success) {
      ...
      log(`New preferences: ${JSON.stringify(updateResult.currentPreferences, null, 2)}`, COLORS.GREEN);
    } else {
    ...
    log(`  Breakthrough rate: ${(perfSummary.summary.breakthroughRate * 100).toFixed(0)}%`, COLORS.GREEN);
    log(`  Error prevention rate: ${(perfSummary.summary.errorPreventionRate * 100).toFixed(0)}%`, COLORS.GREEN);
    
    if (perfSummary.recommendations.length > 0) {
      log('\nRecommendations:', COLORS.YELLOW);
      perfSummary.recommendations.forEach(rec => log(`  • ${rec}`, COLORS.YELLOW));
    }
    ...
    if (presetResult.success) {
      ...
      log(`New modifier: ${presetResult.currentPreferences.persona_threshold_modifier}`, COLORS.GREEN);
    } else {
