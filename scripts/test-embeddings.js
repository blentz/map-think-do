...
async function testEmbeddingGeneration() {
  ...
  try {
    ...
    const testPrompt = {
      ...
      original_prompt: 'How can I optimize the performance of my machine learning model for text classification? I need to improve accuracy and reduce training time.',
    ...
    const testThought = {
      ...
      thought: 'To optimize machine learning model performance, I should consider: 1) Feature engineering to improve data quality, 2) Hyperparameter tuning using grid search or Bayesian optimization, 3) Model architecture optimization, 4) Data augmentation techniques, and 5) Distributed training for large datasets.',
      thought_number: 1,
      total_thoughts: 3,
    ...
    try {
      ...
      if (embeddingCheck.rows.length > 0) {
        ...
        embeddingCheck.rows.forEach(row => {
          console.error(`   - ${row.thought_id} (${row.embedding_model}) created at ${row.created_at}`);
        });
      } else {
      ...
      if (embeddingCheck.rows.length > 0) {
        ...
        if (firstEmbedding.rows.length > 0) {
          ...
          const similarThoughts = await memoryStore.findSimilarThoughtsSemantic(
            embeddingVector, 0.1, 5, testSessionId
          );
          
          console.error(`✅ Semantic search returned ${similarThoughts.length} similar thoughts`);
          similarThoughts.forEach(thought => {
            console.error(`   - Similarity: ${thought.similarity_score?.toFixed(3)} | ${thought.thought_text?.substring(0, 100)}...`);
          });
        }
      }
