...
class PRPMonitoringSystem {
  ...
  async collectRealTimeMetrics() {
    ...
    try {
      ...
      metrics.database_health = {
        ...
        avg_query_time:
          sqlResult.metrics.reduce((sum, m) => sum + m.execution_time_ms, 0) /
          sqlResult.metrics.length,
      };
  ...
  extractMetricValue(metrics, name) {
    ...
    return metric
      ? typeof metric.value === 'string' && metric.value.startsWith('ERROR:')
        ? 0
  ...
  checkAlerts(metrics) {
    ...
    if (
      metrics.performance_metrics.classification_accuracy <
      this.alertThresholds.classification_accuracy
    ) {
      alerts.push({
    ...
    if (
      metrics.performance_metrics.intent_extraction_precision <
      this.alertThresholds.intent_extraction_precision
    ) {
      alerts.push({
    ...
    if (
      metrics.performance_metrics.similarity_detection_recall <
      this.alertThresholds.similarity_detection_recall
    ) {
      alerts.push({
  ...
  displayMetrics(metrics) {
    ...
    log(
      `  Memory Usage: ${(metrics.system_health.heap_used / 1024 / 1024).toFixed(1)}MB / ${(metrics.system_health.heap_total / 1024 / 1024).toFixed(1)}MB`,
      colors.cyan
    );
    log(
      `  Memory Efficiency: ${(metrics.system_health.memory_efficiency * 100).toFixed(1)}% ${metrics.system_health.memory_efficiency <= this.alertThresholds.memory_efficiency ? '✅' : '⚠️'}`,
      colors.cyan
    );
    ...
    log(
      `  Processing Success Rate: ${(metrics.database_health.processing_success_rate * 100).toFixed(1)}%`,
      colors.cyan
    );
    log(
      `  Average Query Time: ${metrics.database_health.avg_query_time.toFixed(1)}ms ${metrics.database_health.avg_query_time <= this.alertThresholds.query_response_time ? '✅' : '⚠️'}`,
      colors.cyan
    );
    log('');
    ...
    log(
      `  Classification Accuracy: ${(metrics.performance_metrics.classification_accuracy * 100).toFixed(1)}% ${metrics.performance_metrics.classification_accuracy >= this.alertThresholds.classification_accuracy ? '✅' : '❌'}`,
      colors.cyan
    );
    log(
      `  Intent Extraction Precision: ${(metrics.performance_metrics.intent_extraction_precision * 100).toFixed(1)}% ${metrics.performance_metrics.intent_extraction_precision >= this.alertThresholds.intent_extraction_precision ? '✅' : '❌'}`,
      colors.cyan
    );
    log(
      `  Similarity Detection Recall: ${(metrics.performance_metrics.similarity_detection_recall * 100).toFixed(1)}% ${metrics.performance_metrics.similarity_detection_recall >= this.alertThresholds.similarity_detection_recall ? '✅' : '❌'}`,
      colors.cyan
    );
    log(
      `  Reasoning Improvement: ${(metrics.performance_metrics.reasoning_improvement_average * 100).toFixed(1)}%`,
      colors.cyan
    );
    log(
      `  Storage Performance: ${metrics.performance_metrics.storage_performance_ms.toFixed(1)}ms`,
      colors.cyan
    );
    log('');
    ...
    log(
      `  System status: ${metrics.alerts.filter(a => a.severity === 'error').length === 0 ? 'HEALTHY' : 'DEGRADED'}`,
