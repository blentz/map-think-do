...
const displayConfig = (config, specs) => {
  ...
  console.log(
    `   Consciousness Processing: ${config.consciousnessProcessingInterval}ms (${(config.consciousnessProcessingInterval / 1000).toFixed(1)}s)`
  );
  console.log(
    `   Stream Generation: ${config.streamGenerationInterval}ms (${(config.streamGenerationInterval / 1000).toFixed(1)}s)`
  );
  console.log(
    `   Memory Monitoring: ${config.memoryMonitoringInterval}ms (${(config.memoryMonitoringInterval / 1000).toFixed(1)}s)`
  );
...
const main = () => {
  ...
  switch (command) {
    ...
    case 'benchmark':
      ...
      console.log(
        `   Estimated runtime before cleanup: ${(config.maxThoughtHistory / thoughtsPerMinute).toFixed(1)} minutes`
      );
      break;
